...
namespace Fluxer.Helpers
{
    internal static class HelperMethods
    {
        ...
        public static async Task<byte[]> GetCachedAvatarAsync(string userId, string hash, FluxerChannelType channel_type)
        {
            string cacheDirName = channel_type == FluxerChannelType.Group ? "group"
                    : channel_type == FluxerChannelType.Server ? "server"
                    : "user";
        ...
        private static string GetAvatarUrl(string Id, string Hash, FluxerChannelType channel_type)
        {
            if (channel_type == FluxerChannelType.Server)
                return $"https://cdn.fluxerapp.com/icons/{Id}/{Hash}.png?size={AVATAR_SIZE}";

            else if (channel_type == FluxerChannelType.Group)
                return $"https://cdn.fluxerapp.com/channel-icons/{Id}/{Hash}.png?size={AVATAR_SIZE}";

            else return $"https://cdn.fluxerapp.com/avatars/{Id}/{Hash}.png?size={AVATAR_SIZE}";
        }
    }
}
